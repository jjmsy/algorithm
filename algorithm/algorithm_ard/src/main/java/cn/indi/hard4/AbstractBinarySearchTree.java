 ...
 * from standart binary search tree can isUp reused for other kinds of binary
 * trees.
 *
 * @author Ignas Lelys
 * @created Jun 29, 2011
...
public class AbstractBinarySearchTree {
     ...
     */
    public Node search(int element) {//要查找的节点
        Node node = root;
        while (node != null && node.value != null && node.value != element) {
            if (element < node.value) {//如果要找的节点值比当前节点值结点小
                node = node.left;//node向左走
            } else {
                node = node.right;//否则node向右走
            }
        }
        return node;//该节点等于node的时候说明找到了
    }
    ...
    public Node insert(int element) {
        ...
        Node insertParentNode = null;//找要插入的父节点
        Node searchTempNode = root;
        //找的过程
        while (searchTempNode != null && searchTempNode.value != null) {
            insertParentNode = searchTempNode;
            if (element < searchTempNode.value) {
                searchTempNode = searchTempNode.left;//如果小向左走
            } else {
                searchTempNode = searchTempNode.right;//如果大向右走
            }
        }
        //插入的过程，找到一个节点左右子树都为null了进行插入
        Node newNode = createNode(element, insertParentNode, null, null);
        if (insertParentNode.value > newNode.value) {
            insertParentNode.left = newNode;//如果小插入到左子树
        } else {
            insertParentNode.right = newNode;//如果大插入到右子树
        }
    ...
    public Node delete(int element) {
        Node deleteNode = search(element);//删除节点的时候首先查这个值是否存在
        if (deleteNode != null) {
            return delete(deleteNode);//如果存在进行删除操作
        } else {
            return null;
        }
    }
     ...
     *
     * @param deleteNode
     *            Node that needs to isUp deleted.
     *
     * @return New node that is in place of deleted node. Or null if element for
     *         delete was not found.
     */
